...
| 구분 | CORS | CSRF |
|------|------|------|
| **문제** | 다른 도메인 API 호출 차단 | 의도하지 않은 요청 실행 |
| **누가 막나** | 브라우저 | 서버 |
| **해결 방법** | 서버에서 허용 설정 | 토큰 검증 또는 JWT 사용 |
| **현재 상태** | ✅ 미리 설정됨 (CorsConfig.java) | ✅ JWT로 해결됨 |
...
| 상황 | 필요 여부 |
|------|----------|
| 현재 (Spring이 HTML+API 제공) | ❌ 불필요 |
| 나중에 (Nginx가 HTML, Spring이 API만) | ✅ 필요! |
| Flutter 앱에서 API 호출 | ❌ 앱은 브라우저 아님 |
| React/Vue 개발 서버 (localhost:3000) | ✅ 필요 |
...
| 항목 | 설명 | 현재 상태 |
|------|------|----------|
| **CORS** | 다른 도메인 API 호출 허용 | ✅ 미리 설정됨 (CorsConfig.java) |
| **CSRF** | 위조 요청 방어 | ✅ JWT로 해결됨 |

### 🔮 CORS가 필요해지는 시점

```
지금: Spring이 HTML + API 모두 제공 → 같은 도메인이라 CORS 불필요
      (하지만 미리 설정해둠!)
     
나중에: Nginx(HTML) + Spring(API만) 분리 → CORS 필수!
        Spring에서 Thymeleaf, HTML 다 빼고
        @RestController만 남길 때!
```
